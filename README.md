...
- **Undoing an accepted suggestion:** Undo restores the note's text *and* reopens the suggestion, so it can be acted on again; redo accepts it once more. Accepting is its own undo step, so undo never takes back what you typed just before it. If you decided the suggestion some other way in between, undo leaves that decision alone and only restores the text.
